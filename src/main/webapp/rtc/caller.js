...
function onSignalMessage(message){
    ...
    if(JSON.parse(message).sdp){
      ...
      localPeerConnection.setRemoteDescription(new RTCSessionDescription(JSON.parse(message)));
      return;
    } else if(JSON.parse(message).candidate) {
      console.log("signal had candidate");
      localPeerConnection.addIceCandidate(new RTCIceCandidate(JSON.parse(message)));
      return;
    }
}
...
function gotLocalDescription(desc) {
  ...
  gaeChannel.sendMessage(gaeChannel.peer, JSON.stringify(desc));
...
function gotLocalCandidate(event) {
  ...
  if (event.candidate) {
    ...
    gaeChannel.sendMessage(gaeChannel.peer, JSON.stringify(event.candidate));
